...
class Cactus(pygame.sprite.Sprite):
    def __init__(self, x, y, pos):
        ...
        self.rect = self.image.get_rect()
        self.rect.topleft = [x, HEIGHT - 175]
...
def reset_game():
    global score, scroll_speed, game_over, last_cactus, game_over
    score = 0
    scroll_speed = initial_scroll_speed
    game_over = False
    last_cactus = pygame.time.get_ticks() - cactus_frequency
    trex1 = Trex(int(WIDTH / 4), 200)
    trex_group.add(trex1)
...
while running:
    if game_over: 
        ...
        last_cactus = cactus_group.sprites()[-1]if cactus_group.sprites() else None
        print(last_cactus)
        last_cactus = None
        cactus_group.empty()
        trex_group.empty()
